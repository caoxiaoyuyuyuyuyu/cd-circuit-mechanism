#!/usr/bin/env python3
"""exp-00v2: CD Baseline Revised Evaluation
Fixes 3 evaluation flaws from exp-00:
...
def greedy_decode_cd(expert_model, amateur_model, tokenizer, prompt, alpha, device, max_tokens=MAX_GEN_TOKENS):
    """Greedy decode using CD logits. alpha=0 falls back to expert-only."""
...
def eval_triviaqa(expert_model, amateur_model, tokenizer, dataset, train_data, alpha, device):
    """Evaluate TriviaQA with 5-shot prompting."""
    ...
    for idx, item in enumerate(dataset):
        ...
        pred_normalized = normalize_answer(response.split("\n")[0].strip())
        ...
        per_sample.append({
            ...
            "cd_score_correct": 0.0,
            "cd_score_top": 0.0,
            "expert_logp_correct": 0.0,
            "cd_logp_correct": 0.0,
            "delta_logp": 0.0,
        })
...
def eval_gsm8k(expert_model, amateur_model, tokenizer, dataset, train_data, alpha, device):
    """Evaluate GSM8K with 5-shot CoT prompting."""
...
def main():
    ...
    save_result("exp00v2_truthfulqa_shuffle_verify.json", verify_result)
    logger.info(f"SHUFFLE VERIFY: accuracy={verify_result['metrics']['accuracy']:.3f} "
                f"(exp-00 unshuffled was ~0.995)")
    ...
    for alpha in ALPHAS:
        ...
        save_result(f"exp00v2_truthfulqa_alpha{alpha}.json", tqa_result)
        ...
        save_result(f"exp00v2_hellaswag_alpha{alpha}.json", hs_result)
        ...
        tqa_open_result = eval_triviaqa(expert_model, amateur_model, tokenizer,
                                         ...
                                         alpha, device)
        save_result(f"exp00v2_triviaqa_alpha{alpha}.json", tqa_open_result)
        ...
        gsm_result = eval_gsm8k(expert_model, amateur_model, tokenizer,
                                 ...
                                 alpha, device)
        save_result(f"exp00v2_gsm8k_alpha{alpha}.json", gsm_result)
    ...
    for alpha in ALPHAS:
        ...
        for ds in ["truthfulqa", "hellaswag", "triviaqa", "gsm8k"]:
            fname = f"exp00v2_{ds}_alpha{alpha}.json"
